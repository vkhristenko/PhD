...
%gets a total weight of zero when the boundary is computed.
...
%given by the second element in the tensor.
...
%    \put(-300,130){$(p+r-u, q+s-v)$}
%    \put(-100,-20){$(p-r+u, q-s+v)$}
%    \put(-350,-20){$(p-r-u, q-s-v)$}
%    \put(-120, 10){$t^{-\alpha}$}
%    \put(-80, 115){$t^{\alpha}$}
%    \put(-140, 40){$t^{\beta}$}
%    \put(-120, 90){$t^{-\beta}$}
...
%Figure 6:  A parallelogram as a boundary
%  \end{center}
%\end{slide}
...
%\]
%Thus, in the quotient $Z_1(A)/B_1(A)$, we
...
%y \otimes (0,1)_T,$$ with $x,y \in A$.

%Therefore any
%element in $HH_1(A)$ can be written as a
...
%& & - \lambda_{p-1} (p-1,q)_T \ot (0,1)_T \nonumber \\
%& & - \lambda_{q-1} (p,q-1)_T \ot (1,0)_T. \nonumber
%\end{eqnarray}
%\end{definition}
...
%    \put(-135,220){$-\lambda_{p-1}$}
%    \put(-80,265){$\lambda_{q+1}$}
%    \put(-90,180){$-\lambda_{q-1}$}
%    \put(-40,120){$p+1$}
%    \put(-110,120){$p-1$}
%    \put(-195,250){$q+1$}
%    \put(-195,200){$q-1$}
...
%There is a map
%$$\Phi : K_{-1}(M) \to R(M),$$ where $R(M)$ is the coordinate ring of
%$\slc$-characters of $\pi_1(M)$.  The kernel of $\Phi$
%is the nilradical of $K_{-1}(M)$.
%\label{bullock}
%\end{theorem}

%To get this map, Bullock connected the $SL_2(\mathbb{C})$ trace identity
%$$\mathrm{tr}(AB) + \mathrm{tr}(AB^{-1}) = \mathrm{tr}(A)\mathrm{tr}(B)$$
%to the Kauffman bracket relation
%$$\lcr-t\zer-t^{-1}\ift$$ when $t = -1$.
...
%\begin{slide}
%\textbf{example, the three-sphere}
...
%Indeed, $K_{-1}(T^2) = \bbc[x,y,z] / I$ where $I$ is the ideal
%$I = (x^2+y^2+z^2+xyz-4)$.
...
\begin{slide}
  \begin{center}
    \epsfxsize = 18cm
    \epsfysize = 7.5cm
    \epsfbox{figs/skein-delta-slide.eps}
    \put(-420, 184){$=t$}
    \put(-277, 184){$+t^{-1}$}
    \put(-420, 102){$=t^2$}
    \put(-277, 102){$+$}
